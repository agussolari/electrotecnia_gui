...
def generate_input_signal(self):
    ...
    if self.feSenoide_button.isChecked():
        ...
        if a0 !=0 and f0 !=0:
            ...
            self.MplWidget.canvas.axes.plot(isig.input_signal.tt, isig.input_signal.st, color = "blue", label="Entrada")
            osig.processEntry(self)
        ...
        if a0 !=0  and f0 !=0:
            isig.input_signal.tt, isig.input_signal.st = isig.generate_square_signal(100, float(f0), 100, float(a0), 0, float(duty))
            self.MplWidget.canvas.axes.plot(isig.input_signal.tt, isig.input_signal.st, color = "blue", label="Entrada")
            osig.processEntry(self)
    ...
    self.MplWidget.canvas.axes.legend(loc='lower left', shadow=True, fontsize='small', frameon=False)
